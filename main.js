...
// 2026-08-14 수정: "핵심 키워드" 단문 배치 방식 → 롱테일 키워드(3단어 이상
// 구체적 조합) 방식으로 전환. 배경 — 저지수(최근 발행 이력이 짧은) 블로그는
// 경쟁이 치열한 짧은 핵심 키워드로는 상위 노출이 어렵고, 검색 의도가 뚜렷한
// 롱테일 키워드가 신생/저지수 블로그에 유리하다는 SEO 조언에 따라 사용자가
// 요청. 동시에 "AI가 기계적으로 만든 것 같은" 정형화된 제목 패턴("완벽 정리",
// "TOP 10" 등)을 피하고 실제 블로거가 자연스럽게 붙인 듯한 어투로 쓰도록
// 규칙 추가.
// 2026-08-20 수정: 기존 제목 규칙이 "검색엔진이 정확히 찾는 제목"(SEO 롱테일)
// 위주였고 "사람이 클릭하고 싶어지는 제목"(궁금증 유발)은 예시 수준으로만
// 권장돼 실제로는 잘 반영 안 되던 문제 — 사용자가 "블로그 운영자들이 흔히
// 말하는 '읽고 싶은 글을 써라·궁금증을 유발하는 제목을 써라' 원칙에 지금
// 규칙이 부합하는가" 질문, 부족하다고 진단 후 합의. 시의성과 무관해 이슈성/
// 에버그린 어디에나 통하는 후킹 기법 4종(질문형 마무리/정보 격차/구체적
// 수치 대비/상황·시점 긴장감)을 정의해 "최소 1개 이상 필수 반영"으로 격상.
const TITLE_RULE_BLOCK = `- [제목 작성 규칙 - SEO 최적화, 롱테일 키워드]
  · 제목 구성 공식: [롱테일 키워드] + [타겟층·상황] + [매력적인 후킹 포인트] 조합으로 작성
  · [롱테일 키워드 필수] 짧은 단일 키워드(예: "반도체 투자")를 그대로 제목에
    쓰지 말 것. 위에 주어진 키워드와 주제를 조합해, 검색 의도가 뚜렷하게
    드러나는 3단어 이상의 구체적인 조합(예: "반도체 투자 시기 놓쳤을 때
    대처법"처럼 상황·조건·대상이 담긴 형태)으로 만들어 제목 앞부분에 배치할 것
  · 길이는 30~50자
  ...
  · [AI스러운 정형화 패턴 금지 - 사람이 쓴 것처럼, 2026-08-14 신규] "완벽 정리",
    "총정리", "TOP 10", "~하는 이유 5가지", "몰랐던 꿀팁", "필수 아이템"처럼
    뻔한 SEO·어그로형 템플릿을 기계적으로 붙이지 말 것. 실제 블로거가 자기
    경험·상황에서 자연스럽게 떠올려 붙인 듯한 어투로 쓸 것
  · [궁금증 유발 기법 - 최소 1개 이상 필수 반영, 2026-08-20 신규] "내가 쓰고
    싶은 글"이 아니라 "남이 읽고 싶은 글"이 되려면 제목만 보고도 클릭하고
    싶어져야 함. 아래 4가지 기법 중 최소 1개 이상을 자연스럽게 녹여 쓸 것
    (전부 시의성과 무관한 기법이라 이슈성·에버그린 주제 어디에나 동일하게
    적용할 것):
    1) 질문형 마무리 — 답을 다 주지 않고 여운을 남기는 질문으로 끝맺기
       (예: "~해도 될까?", "~어떻게 해야 할까?")
    2) 정보 격차 — 제목에 결론·방법·수치를 전부 드러내지 말고, 본문에서
       풀릴 내용을 예고만 할 것(예: "~하는 이유"는 이유를 예고만 하고
       구체적인 내용은 본문에서 밝힐 것 — 제목만 보고 답이 다 유추되게
       하지 말 것)
    3) 구체적 수치를 통한 대비 — "TOP 10"류 리스트형은 계속 금지하되,
       "2주 만에", "3번째 시도에서"처럼 문장 속에 자연스럽게 녹인 구체적
       숫자는 신빙성과 궁금증을 동시에 주므로 허용/권장
    4) 상황·시점 기반 긴장감 — 위 "타겟층·상황" 요소를 살려 "이게 내
       얘기인가?" 하는 긴장감이 들도록 구체적인 순간·조건을 짚을 것
       (예: "~때", "~시기", "~직전/직후")
  · 예: "반도체 투자 시기 놓쳤을 때 대처법, 지금 시작해도 될까?"`;
...
`
  ...
  // 2026-07-24 신규(사용자 요청): 리뷰형 톤일 때만 제휴마케팅에 특화된
  // 구조 지침을 추가("가벼운 버전" — 기존 ##/###/####/▪ 렌더링 구조는
  // 그대로 유지하고 AI 글쓰기 지침만 강화). 사용자가 참고로 보여준 원안
  // (제품 개요/경쟁 제품 비교를 표로 정리)은 채택하지 않음 — 실제
  // 삽입되는 제휴 상품(쿠팡/알리 API 검색 결과)은 글 내용과 완전히
  // 별개로 결정되는데, AI가 특정 제품의 구체적 스펙・가격을 서술하면
  // 실제 삽입된 상품과 안 맞는 경우가 생길 위험이 크다고 판단(사용자와
  // 협의 완료) — 그래서 특정 제품을 단정적으로 설명하는 대신 일반적인
  // 선택 기준・사용 경험 위주로 쓰도록 유도.
  const reviewToneGuide = tone === 'review' ? `
- [리뷰형 전용 구조 지침 - 2026-07-24 신규, 제휴마케팅 특화]
  · 도입부는 독자의 고민이나 궁금증에 먼저 공감하며 시작할 것(예: "~때문에
    고민이셨다면" 같은 화법으로 독자의 상황을 먼저 짚어줄 것)
  · 본문 중 최소 한 구간은 실제로 써본 사람의 시점에서 구체적인 사용 경험
    (E-E-A-T: 실제 경험・전문성이 느껴지는 디테일)을 담을 것 — 막연한
    칭찬이 아니라 "이럴 때 특히 좋았다/아쉬웠다"처럼 구체적 상황 묘사
  · 장점만 나열하지 말고 단점이나 아쉬운 점도 최소 1가지 이상 솔직하게
    포함할 것 — 이는 신뢰도를 높이는 리뷰의 핵심 요소
  · [중요] 특정 제품의 정확한 스펙・가격・모델명을 단정적으로 서술하지
    말 것 — 본문과 별개로 실제 제휴 상품이 자동으로 삽입되므로, 서로
    다른 제품처럼 보일 위험이 있음. 대신 "이런 기능/특징을 가진 제품을
    고를 때"처럼 일반적인 선택 기준 위주로 서술할 것
  · 마무리는 지금까지 설명한 선택 기준이나 후기를 바탕으로, 독자가 직접
    확인해보고 싶어지도록 자연스럽게 유도하는 문장으로 끝맺을 것(과도한
    광고 문구나 직접적인 구매 강요 표현은 피할 것)
` : '';
