...
다음 JSON 형식으로만 응답하세요 (모든 값은 반드시 순수 한국어):
{
  ...
  "thumbText": "썸네일 이미지에 들어갈 별도 문구 — 공백 제외 글자 수를 반드시 세어서 정확히 18~20자 범위로 작성(17자 이하·21자 이상 금지), 핵심 키워드 최소 1개 포함, 제목과 똑같은 문장이 아니라 더 짧고 임팩트 있게 축약, 조사나 연결어미(~는/~와/~에/~하고 등)로 끝나 미완성처럼 보이지 않도록 반드시 자연스럽게 완결된 형태로 마무리, 마크다운 기호 없는 한 줄 평문, 한글만",
...
현재는 ${new Date().getFullYear()}년입니다 — 키워드에 연도를 포함할 경우 반드시 이 연도를
기준으로 하고, 오래된 연도(예: 2024년)를 쓰지 마세요.
